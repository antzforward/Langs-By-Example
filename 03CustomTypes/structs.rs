...
impl Rectangle {
	fn rect_area(&self)->f32{
		...
		let height = self.top_left.x - self.bottom_right.y;
		( width * height).abs()
    }
    fn square(point:Point,width:f32)->Rectangle{
		Rectangle{ top_left:Point{x:point.x, y:point.y},
...
fn main() {
	...
	println!("rectangle area:{}",Rectangle::square(Point{x:0.0,y:0f32}, 35.0f32).rect_area());
    // 实例化一个单元结构体
    let _unit = Unit;
